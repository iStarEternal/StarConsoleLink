...
var UsedConsoleKey = "isConsoleKey"
...
extension NSTextStorage {
    ...
    var usedInConsole: Bool {
        get {
            return (objc_getAssociatedObject(self, &UsedConsoleKey) as? NSNumber)?.boolValue ?? false
        }
        set {
            objc_setAssociatedObject(self, &UsedConsoleKey, NSNumber(bool: newValue), .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    func applyANSIColors(textStorageRange: NSRange, escapeSeq: String) {
        LogColorKit.applyANSIColors(self, textStorageRange: textStorageRange, escapeSeq: escapeSeq)
    }
